...
### Purist Page Object Model (POM)
Adhering to clean code standards outlined by Martin Fowler, this framework applies a **Purist Page Object Model** pattern. 
...
### Runtime Authentication (Global Setup)
Instead of executing authentication flows inside every test suite or maintaining hardcoded sessions, the framework utilizes a centralized `global-setup.ts` mechanism. 
* It authenticates once at the beginning of the runtime execution via headless interaction.
* It securely exports the browser state to a git-ignored state file (`playwright/.auth/user.json`).
* Secured projects (`chromium-secure`) automatically inject this file as their `storageState`, eliminating login overhead and maximizing execution speed.
...
* **`.github/copilot-instructions.md`**: Provides explicit context to GitHub Copilot to strictly follow the Purist Page Object Model, utilize `pnpm`, and enforce Playwright Web-First accessibility locators.
...
* `pnpm run inspect`: Launches the Playwright Codegen tool (`playwright codegen`) pointing to the target testing environment. It allows engineers to record interactions, generate robust selectors, and export code snippets rapidly.
...
│       │   └── login.spec.ts
│       └── secure/             # Protected area verification tests (Injected Auth)
