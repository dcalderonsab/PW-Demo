...
A senior-grade End-to-End (E2E) automation testing framework designed for scalability, maintainability, and enterprise-level resilience. This framework is built using **Playwright**, **TypeScript**, and **Docker**, utilizing **pnpm** as its package manager.
...
6. [Docker Containerization Architecture](#6-docker-containerization-architecture)
7. [AI-Assisted Development & CLI Tooling](#7-ai-assisted-development--cli-tooling)
...
To completely eradicate the *"It works on my machine"* syndrome, the entire framework is fully containerized, making it immediately ready for CI/CD integration.

### Exact Version Pinned Matching
Following enterprise standards, the `Dockerfile` image base is explicitly pinned to match the exact Playwright engine version defined in `package.json` (`v1.60.0`):
```dockerfile
FROM mcr.microsoft.com/playwright:v1.60.0-jammy
```
This ensures that the underlying browser binaries (Chromium, Firefox, WebKit) and OS-level rendering libraries are 100% identical across local environments, peer reviews, and remote pipelines.

### Corepack Cache Layer Optimization
The Dockerfile leverages Docker’s layer caching system sequentially. Dependency management manifests (`package.json` and `pnpm-lock.yaml`) are isolated and installed via `pnpm` using `corepack` *before* copying the remaining source code. This drastically reduces subsequent image build times.

### Critical Execution Parameters (IPC & Secrets)
When executing the containerized environment, specific flags are utilized to maintain stability and security:
* **`--ipc=host` (Mandatory):** Modern automated browsers consume significant shared memory. Docker containers default to a restricted allocation, which causes unexpected browser crashes. This flag maps the container to the host's IPC namespace, unlocking full memory allocation.
* **`--env-file .env`:** Passwords and environment base URLs are securely injected at runtime into the container. This preserves compliance standards by keeping production-ready image layers completely decoupled from static secrets.

---

## 7. AI-Assisted Development & CLI Tooling
...
├── .dockerignore               # Optimizes image compilation footprint
├── .env.example                # Blank environment variable blueprint
├── .gitignore                  # Prevents artifacts and sessions leaks
├── Dockerfile                  # Container infrastructure instructions
...
### Containerized Execution (Docker Engine)
To run the automation framework inside its isolated, cross-platform container network without installing local dependencies, execute the following commands in order:

1. **Build the Container Image:**
   ```bash
   docker build -t pw-assessment .
   ```

2. **Execute the Suite with Secure IPC Mapping and Environment Injection:**
   ```bash
   docker run -it --rm --ipc=host --env-file .env pw-assessment
   ```

*Review Note:* If tests encounter any failure inside the container, diagnostic screenshots and traces will be compiled safely within the isolated container space. To persistently view them on the host machine, you can mount a local directory volume utilizing standard `-v` execution flags.
